...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!passwordMatch) return res.status(401).json({ message: 'Identifiants invalides.' });
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (!user || !user.twoFactorEnabled) return res.status(401).json({ message: 'Utilisateur introuvable ou 2FA non configuré.' });
      ...
      if (!verified) return res.status(401).json({ message: 'Code 2FA invalide.' });
      ...
      res.json({ token: authToken, user: { id: user.id, email: user.email, name: user.name, points: user.points, role: user.role } });
    } catch (error) {
      next(error);
    }
  }
...
router.post('/2fa/setup', verifyToken, async (req, res, next) => {
  try {
    const secret = speakeasy.generateSecret({ name: `IS Beauty (${req.user.email})` });
    const qrCodeUrl = await qrcode.toDataURL(secret.otpauth_url);
    await db.run('UPDATE users SET twoFactorSecret = ? WHERE id = ?', secret.base32, req.user.id);
...
// ── 2FA ENABLE ──
router.post('/2fa/enable', verifyToken, body('token').isString().notEmpty(), async (req, res, next) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) return res.status(400).json({ errors: errors.array() });

    const user = await db.get('SELECT * FROM users WHERE id = ?', req.user.id);
    if (!user?.twoFactorSecret) return res.status(400).json({ message: '2FA non configuré.' });
...
router.get('/me', verifyToken, async (req, res, next) => {
  try {
    const user = await db.get('SELECT id, email, name, points, role, twoFactorEnabled FROM users WHERE id = ?', req.user.id);
    res.json({ user });
  } catch (error) {
    next(error);
  }
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      await db.run('UPDATE users SET passwordHash = ? WHERE id = ?', newHash, user.id);
...
router.patch(
  ...
  async (req, res, next) => {
    try {
      ...
      const user = await db.get('SELECT id, email, name, points, role FROM users WHERE id = ?', req.user.id);
  ...
  async (req, res, next) => {
    try {
      ...
      await db.run(
        ...
        uuidv4(), email, resetToken, expiresAt, new Date().toISOString()
      );
